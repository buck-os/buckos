...
impl UseFlags {
    ...
    /// Convert to the shared `buckos_package::UseConfig` type.
    ///
    /// This allows the installer to use the same `sync_config_to_repo()`
    /// code path as the buckos CLI, ensuring a single source of truth
    /// for Starlark generation.
    pub fn to_use_config(&self) -> buckos_package::UseConfig {
        use buckos_package::{PackageId, UseConfig};
        use std::collections::{BTreeMap, HashSet};

        let global: HashSet<String> = self.global.iter().cloned().collect();

        let mut expand = BTreeMap::new();
        if !self.video_cards.is_empty() {
            expand.insert(
                "VIDEO_CARDS".to_string(),
                self.video_cards.iter().cloned().collect(),
            );
        }
        if !self.input_devices.is_empty() {
            expand.insert(
                "INPUT_DEVICES".to_string(),
                self.input_devices.iter().cloned().collect(),
            );
        }

        let mut package = BTreeMap::new();
        for (pkg_name, flags) in &self.package_use {
            let pkg_id = PackageId::new("unknown", pkg_name);
            package.insert(pkg_id, flags.iter().cloned().collect());
        }
    ...
    /// Sync USE flag configuration to the buckos-build repo.
    ///
    /// Delegates to `buckos_package::buck::sync_config_to_repo()` — the same
    /// code path used by the buckos CLI — to write `config/local_modifiers.bzl`
    /// and per-package PACKAGE files.
    pub fn sync_to_repo(&self, buckos_build_path: &std::path::Path) -> anyhow::Result<()> {
        let pkg_config = buckos_package::Config {
            buck_repo: buckos_build_path.to_path_buf(),
            use_flags: self.to_use_config(),
            ..Default::default()
        };

        buckos_package::buck::sync_config_to_repo(&pkg_config)
            .map_err(|e| anyhow::anyhow!("Failed to sync config to buckos-build: {}", e))
    }

    /// Generate TOML format for /etc/buckos/buckos.toml
    pub fn to_toml(&self, profile: &str) -> String {
...
# Edit via: buckos config set <key> <value>
# USE flags: buckos use [flags]
...
"#,
            ...
            video_cards = video_cards_toml,
            input_devices = input_devices_toml,
            package_use = package_use_toml,
            arch = arch,
            chost = chost,
            march = march,
            jobs = jobs
...
"#,
        ...
        let profile_name = match &config.profile {
            ...
            InstallProfile::Handheld(_) => "desktop",
            InstallProfile::Custom => "custom",
        };
        std::fs::write(&buckos_toml_path, target_use_flags.to_toml(profile_name))?;
        tracing::info!(
            "Generated /etc/buckos/buckos.toml with {} USE flags (profile: {})",
